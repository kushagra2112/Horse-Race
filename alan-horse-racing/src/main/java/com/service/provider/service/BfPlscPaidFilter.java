...
public class BfPlscPaidFilter  implements FilterConditions {
	...
	public List<HorseDataResult> isEqual(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> d.getBfPlcsPaid() == Double.parseDouble(value))
	...
	public List<HorseDataResult> isBetween(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> (d.getBfPlcsPaid() >= Double.parseDouble(value.toLowerCase().split(" and ")[0]) 
						&& d.getBfPlcsPaid() < Double.parseDouble(value.toLowerCase().split(" and ")[1])))
				.collect(Collectors.toList());
	}
