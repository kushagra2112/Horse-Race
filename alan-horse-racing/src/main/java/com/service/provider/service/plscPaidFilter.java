...
public class plscPaidFilter implements FilterConditions {
	...
	public List<HorseDataResult> isBetween(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> (d.getPlcsPaid() >= Double.parseDouble(value.toLowerCase().split(" and ")[0]) 
						&& d.getPlcsPaid() < Double.parseDouble(value.toLowerCase().split(" and ")[1])))
				.collect(Collectors.toList());
	}
