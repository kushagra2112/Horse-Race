...
public class RaceDistanceFilter implements FilterConditions {

	@Override
	public List<HorseDataResult> isEqual(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> d.getRaceDistance() == Double.parseDouble(value))
	...
	public List<HorseDataResult> isBetween(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> (d.getRaceDistance() >= Double.parseDouble(value.toLowerCase().split(" and ")[0]) 
						&& d.getRaceDistance() < Double.parseDouble(value.toLowerCase().split(" and ")[1])))
				.collect(Collectors.toList());
	}
