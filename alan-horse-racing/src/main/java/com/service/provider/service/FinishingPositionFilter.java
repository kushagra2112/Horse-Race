...
public class FinishingPositionFilter implements FilterConditions {

	@Override
	public List<HorseDataResult> isEqual(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> d.getFinishingPosition() == Long.parseLong(value))
	...
	public List<HorseDataResult> isBetween(List<HorseDataResult> horseDetails, String value) {
		return horseDetails.stream()
				.filter(d -> (d.getFinishingPosition() >= Long.parseLong(value.toLowerCase().split(" and ")[0]) 
						&& d.getFinishingPosition() < Long.parseLong(value.toLowerCase().split(" and ")[1])))
				.collect(Collectors.toList());
	}
